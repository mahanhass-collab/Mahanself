...
def get_remaining_days_in_year(year, current_month, current_day):
    remaining_days = get_jalali_month_days(year, current_month) - current_day
...
async def update_profile_name(client):
    ...
    while True:
        if time_enabled:
            ...
            time_now_classic = convert_to_classic_font(f"{hour}:{minute:02d}")
...
async def handle_commands(event):
    ...
    if event.text.lower() == "تایم روشن":
        ...
        new_name = re.sub(r'\s*[𝟶-𝟿]{1,2}:[𝟶-𝟿]{2}\s*', '', current_name)
        try:
            await event.client(UpdateProfileRequest(first_name=new_name.strip()))
        except Exception as e:
            print(f"خطا در به‌روزرسانی نام پروفایل: {e}")
...
async def handle_new_message(event):
    ...
    elif event.sender_id in enemies and is_client_active:
        await send_ordered_reply(event, enemy_responses)
    elif event.sender_id in friends and is_client_active:
...
if __name__ == "__main__":
    asyncio.run(main())    "خسته نشدی ننتو گاییدم؟",
    "کیرم شلاقی به ناموس جندت",
    "با ناموست تریسام زدم",
    "برج خلیفه تو مادرت",
    "دو پایی میرم تو کسمادرت",
